...
#PROMPT='%{$fg_bold[green]%}%n@%m%{$reset_color%} %{$fg_bold[white]%}[%D{%a %H:%M:%S}]%{$reset_color%} %{$fg_bold[blue]%}%2~%{$reset_color%} $(my_git_prompt_info)%{$reset_color%}%B»%b '
#PROMPT='%{$fg_bold[green]%}%n@%m%{$reset_color%} %{$fg_bold[white]%}[%D{%a %H:%M:%S}]%{$reset_color%} %{$fg_bold[blue]%}%2~%{$reset_color%} [%?] $(my_git_prompt_info)%{$reset_color%}%B»%b '

RPS1="${return_code} "
RPS1="$RPS1 %{$fg_bold[grey]%}<%{$reset_color%}%{$fg_bold[blue]%}Java $(java -version 2>&1 | head -n 1 | cut -d '"' -f 2)%{$reset_color%}%{$fg_bold[grey]%}>%{$reset_color%}"
